...
#Changelog
    #1) Created class Number to house all functions and objects of number. Made user input a member of number
    #2) Created restart code to fix issues with error validation causing memory issues and retaining incorrect values
    #3) Added leading zero if nothing before decimal
    #4) Added more character validation to make sure that no characters can pass through
    #5) Added check if nothing before decimal to add a leading zero so ErrorCheck() doesnt't return ValueError as it would be whitespace otherwise
